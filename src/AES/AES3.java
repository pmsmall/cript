...
public class AES3 {
	...
	private char[][] rcon = { { 0x00, 0x00, 0x00, 0x00 }, { 0x01, 0x00, 0x00, 0x00 }, { 0x02, 0x00, 0x00, 0x00 },
	...
	char sbox[] = { 0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76,
	...
	char isbox[] = { 0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 0xBF, 0x40, 0xA3, 0x9E, 0x81, 0xF3, 0xD7, 0xFB,
	...
	public AES3(KeySize keySize, byte[] keys) {
		setNkNr(keySize);
		keyExpansion(keys);
	}
	...
	private void keyExpansion(byte[] inkeys) {
		if (inkeys == null || (inkeys.length != 16 && inkeys.length != 24 && inkeys.length != 32))
	...
	private byte[] subWord(byte[] word) {
		return new byte[] { (byte) sbox[word[0] & 0xff], (byte) sbox[word[1] & 0xff], (byte) sbox[word[2] & 0xff],
				(byte) sbox[word[3] & 0xff] };
	}
	...
	public byte[] cipher(byte[] plainText) {
		...
		data = new byte[plainText.length];
...
//
//		// 这是密钥
//		byte[] keys = { 0x01, 0x03, 0x05, 0x07, 0x09, 0x0B, 0x0D, 0x0F, 0x00, 0x02, 0x04, 0x06, 0x08, 0x0A, 0x0C, 0x0E,
//				 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,0x08,0x09,0x00,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F };
//		AES3 aes = new AES3(KeySize.Bits192, keys);
//		// --------加密-------------------------
//
//		// 这是明文
//		String plainText = "11223344556677889900AABBCCDDEEFF";
//		byte[] temp1 = parseHexStr2Byte(plainText);
//		byte[] cipherText = aes.cipher(temp1);
//
//		System.out.println("plainText(明文): " + plainText);
...
//			m.append(" ");
//		}
//		System.out.println("cipherText(密文):" + m);
//
//		// --------解密-------------------------
//		byte[] ret = aes.invCipher(cipherText);
//		String val = new String(ret);
//		System.out.println("invCipher cipherText(将密文解密后的) : " + val);
//
//	}
